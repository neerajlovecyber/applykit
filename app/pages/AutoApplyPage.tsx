              ...
              <div key={idx} className="grid grid-cols-12 p-3 items-center hover:bg-muted/20 transition-colors">
                ...
                <div className="col-span-2">
                  <Badge variant="outline" className={cn(
                    ...
                    (item.status === "submitted" || item.status === "succeeded" || item.status === "applied" || item.status === "completed")
                      ...
                      : item.status === "skipped" ? "bg-amber-500/10 text-amber-400 border-amber-500/30"
                        : item.status === "pending_review" ? "bg-blue-500/10 text-blue-400 border-blue-500/30"
                          : item.status === "running" ? "bg-sky-500/10 text-sky-400 border-sky-500/30"
                            : item.status === "queued" ? "bg-zinc-500/10 text-zinc-400 border-zinc-500/30"
                              : "bg-rose-500/10 text-rose-400 border-rose-500/30"
                  )}>
                    {(item.status === "submitted" || item.status === "succeeded" || item.status === "applied" || item.status === "completed") && <CheckCircle2 className="h-3 w-3" />}
                    {item.status === "skipped" && <SkipForward className="h-3 w-3" />}
                    {item.status === "pending_review" && <Clock className="h-3 w-3" />}
                    {item.status === "running" && <Clock className="h-3 w-3 animate-spin" />}
